...
public class ProducerConfigurationTests {
    ...
    public void testSendMessageWithNonDefaultKeyAndValueEncoders() throws Exception {
        final ProducerMetadata<String, String> producerMetadata =
                new ProducerMetadata<String, String>("test", String.class, String.class, new StringSerializer(),
        ...
        final ArgumentCaptor<ProducerRecord<String, String>> argument =
                (ArgumentCaptor<ProducerRecord<String, String>>) (Object) ArgumentCaptor.forClass(ProducerRecord.class);
    ...
    public void testSendMessageWithDefaultKeyAndValueEncodersAndCustomSerializableKeyAndPayloadObject()
            throws Exception {
        final ProducerMetadata<byte[], byte[]> producerMetadata =
                new ProducerMetadata<byte[], byte[]>("test", byte[].class, byte[].class, new ByteArraySerializer(),
        ...
        configuration.convertAndSend("test", new TestKey("compositePart1", "compositePart2"),
                new TestPayload("part1", "part2"));

        Mockito.verify(producer, Mockito.times(1)).send(Mockito.any(ProducerRecord.class));

        final ArgumentCaptor<ProducerRecord<byte[], byte[]>> argument =
                (ArgumentCaptor<ProducerRecord<byte[], byte[]>>) (Object) ArgumentCaptor.forClass(ProducerRecord.class);
        Mockito.verify(producer).send(argument.capture());
    ...
    public void testSendMessageWithDefaultKeyEncoderAndNonDefaultValueEncoderAndCorrespondingData() throws Exception {
        ...
        final ProducerMetadata<byte[], TestPayload> producerMetadata =
                new ProducerMetadata<byte[], TestPayload>("test", byte[].class, TestPayload.class,
                        new ByteArraySerializer(), new EncoderAdaptingSerializer<TestPayload>(encoder));
        ...
        final ArgumentCaptor<ProducerRecord<byte[], TestPayload>> argument =
                (ArgumentCaptor<ProducerRecord<byte[], TestPayload>>) (Object) ArgumentCaptor
    ...
    public void testSendMessageWithNonDefaultKeyEncoderAndDefaultValueEncoderAndCorrespondingData() throws Exception {
        final AvroReflectDatumBackedKafkaEncoder<TestKey> encoder =
                new AvroReflectDatumBackedKafkaEncoder<TestKey>(TestKey.class);
        final ProducerMetadata<TestKey, byte[]> producerMetadata =
                new ProducerMetadata<TestKey, byte[]>("test", TestKey.class, byte[].class,
                        new EncoderAdaptingSerializer<TestKey>(encoder), new ByteArraySerializer());
        ...
        final ArgumentCaptor<ProducerRecord<TestKey, byte[]>> argument =
                (ArgumentCaptor<ProducerRecord<TestKey, byte[]>>) (Object) ArgumentCaptor
                        .forClass(ProducerRecord.class);
        Mockito.verify(producer).send(argument.capture());
    ...
    public void testSendMessageWithDefaultKeyAndValueEncodersButNonSerializableKeyAndSerializableValue()
        ...
        configuration.convertAndSend("test", new NonSerializableTestKey("compositePart1", "compositePart2"),
                new TestPayload("part1", "part2"));
    }
    ...
    public void testSendMessageWithDefaultKeyAndValueEncodersButSerializableKeyAndNonSerializableValue()
            throws Exception {
        final ProducerMetadata<byte[], byte[]> producerMetadata =
                new ProducerMetadata<byte[], byte[]>("test", byte[].class, byte[].class,
                        new EncoderAdaptingSerializer<byte[]>(new DefaultEncoder(null)),
                        new EncoderAdaptingSerializer<byte[]>(new DefaultEncoder(null)));
        final Producer<byte[], byte[]> producer = Mockito.mock(Producer.class);
        ...
        configuration.convertAndSend("test", new TestKey("compositePart1", "compositePart2"),
                new NonSerializableTestPayload("part1", "part2"));
    }
